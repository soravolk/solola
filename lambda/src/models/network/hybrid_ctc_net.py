...
# stash the original __init__
_orig_init = RelPositionMultiHeadedAttention.__init__

def _patched_init(self, *args, **kwargs):
    # call the original constructor
    _orig_init(self, *args, **kwargs)
    # now inject the same q_norm/k_norm that MultiHeadedAttention does
    # they expect to normalize over each head-dimension of size d_k
    self.q_norm = nn.LayerNorm(self.d_k)
    self.k_norm = nn.LayerNorm(self.d_k)

# override it
RelPositionMultiHeadedAttention.__init__ = _patched_init
