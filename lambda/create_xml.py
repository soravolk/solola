...
def create_musicxml(notes):
    ...
    # Single staff with TAB clef for guitar tablature
    ET.SubElement(attributes, 'staves').text = '1'

    clef_tab = ET.SubElement(attributes, 'clef', number="1")
    ET.SubElement(clef_tab, 'sign').text = 'TAB'
    ET.SubElement(clef_tab, 'line').text = '5'

    # Staff details for the TAB staff with guitar tuning (standard tuning E-A-D-G-B-E)
